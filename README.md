...
tar -xzf vacmcp_v0.8.0_linux_amd64.tar.gz
./vacmcp version          # v0.8.0, the release this binary was built from
```
...
supported codebase-memory-mcp floor moves to 0.11.0. v0.8.0 makes those budgets
adjustable by a Go embedder, because a fixture cannot know how large somebody
else's history is. Ahead of those:
...
| v0.8.0 | embedder-adjustable operation budgets: variadic options on the git, Zoekt, codebase-memory-mcp and resolver constructors, defaults unchanged — done |
...
The operations row carries no version number on purpose. It was written here as
v0.6.0 and that number went to commit history instead; which release it lands in
is not decided, and guessing one again would only make this table wrong a second
time. v1.0.0 keeps its number because what it names is a semver commitment rather
